...
export default function MultiSelect<T extends MultiSelectOption>(props: MultiSelectProps<T>) {
  ...
  const arrField: any[] = rootObject[props.field];
  ...
  return (
    <>
      <select
        ...
        onChange={(s) => {
          setSelectedOption(props.options.filter(i => i.id == s.target.value)[0]!);
        }}
      >
      ...
      <table className="table-bordered mt-3 w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-center text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            {Object.values(props.columnMap).map((c, i) => <th key={i}>{c}</th>)}
          </tr>
        </thead>
        <tbody className="text-center">
          {arrField && arrField.length != 0 && arrField.map((i: any) =>
            <tr key={hash(i)}>
              {Object.keys(props.columnMap).map(v => <td key={hash(i) + "." + v}>{i[v]}</td>)}
            </tr>)}
        </tbody>
      </table>
    </>
